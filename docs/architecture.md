...
*   **Отслеживание запущенных агентов:** Go-сервер будет поддерживать внутреннюю карту (map) или структуру данных, связывающую `agent_id` с объектом `*os.Process` или его PID. Для обеспечения потокобезопасного доступа к этой карте будет использоваться `sync.RWMutex`. Это позволит отслеживать активные процессы.
...
*   **Сообщение об ошибках:** Все функции инструментов MCP-сервера будут возвращать `error` в случае сбоя. Эти ошибки будут содержать достаточно информации для диагностики (например, "агент с ID X не найден", "не удалось записать файл конфигурации: <причина>"). Эти ошибки будут передаваться обратно MCP-клиенту. Будут использоваться кастомные типы ошибок для более детальной информации, например, `ErrAgentNotFound`, `ErrConfigWriteFailed`.
...
### 8. Паттерн ServerOption

*   **Гибкая конфигурация:** При создании экземпляра `MCPServer` будет использоваться паттерн `ServerOption`. Это позволяет передавать функции-опции (например, `WithLogging()`, `WithInstructions()`) в конструктор `NewMCPServer`, что делает его более гибким и расширяемым без изменения сигнатуры конструктора. Это соответствует принципу открытости/закрытости.
</content>
</content>
